...
public class TrainingAdminServiceImpl implements TrainingAdminService {
    ...
    public Page<TrainingListItemResponse> getTrainingList(
            ...
            String keyword,
            Pageable pageable) {
        Page<Training> trainingPage = trainingRepository.searchTrainings(type, programId, keyword, pageable);
        ...
        List<ProgramTraining> mappings = programTrainingRepository.findByTraining_TrainingIdIn(trainingIds);
    ...
    public TrainingDetailResponse getTrainingDetail(Long trainingId) {
        ...
        List<AssignedProgramResponse> assignedPrograms = programTrainingRepository.findByTraining_TrainingId(trainingId)
                .stream()
                .filter(pt -> pt.getProgram() != null && !pt.getProgram().isDeleted())
                .map(AssignedProgramResponse::from)
                .toList();

        List<FileResponse> files = fileRepository.findByRefTypeAndRefId(RefType.TRAINING, trainingId).stream()
    ...
    public TrainingDeleteResponse deleteTraining(Long trainingId) {
        ...
        long activeCount = programTrainingRepository.countByTraining_TrainingIdAndProgram_DeletedFalse(trainingId);
    ...
    public ProgramTrainingAssignResponse assignTrainingToProgram(
        ...
        boolean alreadyAssigned = programTrainingRepository.existsByProgram_ProgramIdAndTraining_TrainingId(programId,
                trainingId);
    ...
    public ProgramTrainingsResponse getProgramTrainings(
        ...
        List<ProgramTrainingItemResponse> trainings = programTrainingQueryRepository.findProgramTrainings(programId,
    ...
    private void notifyTrainingAttachmentUpdate(Training training, Long trainingId) {
        List<ProgramTraining> programTrainings = programTrainingRepository.findByTraining_TrainingId(trainingId);
